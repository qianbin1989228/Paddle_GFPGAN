# Paddle_GFPGAN
[飞桨论文复现挑战赛第六期](https://github.com/PaddlePaddle/Paddle/issues/41482)  复现论文《[GFP-GAN: Towards Real-World Blind Face Restoration with Generative Facial Prior](https://paperswithcode.com/paper/towards-real-world-blind-face-restoration)》(GFPGAN)
...
总体来看，GFPGAN的思想是建立在[StyleGAN2](https://github.com/NVlabs/stylegan2)基础上的。StyleGAN2是专门用来生成高清人脸图像的算法，其生成的人脸图像更加真实、更加清晰。由于StyleGAN2的输入是一个随机隐向量和噪声，因此如果能替代掉这个随机隐向量和噪声，或者用另一个模型来生成这个隐向量和噪声，那么就可以控制待生成的人像面部特征，GPEN就采用了这样的方法。

## 2. 复现精度
| NetWork | epochs | opt | image_size | batch_size | dataset | PSNR | FID | LPIPS |
| --- | --- | --- | --- | --- | --- | --- | --- |--- |
| GFPGAN |  |  | 512X512 |  | CelebA-HQ | 待完成 |待完成 |待完成 |
